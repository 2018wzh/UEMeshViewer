...
void AObjMeshActor::ClearMesh()
{
    MeshComponent->ClearAllMeshSections();
    MeshBounds = FBox(EForceInit::ForceInit);
}

bool AObjMeshActor::LoadFromObjFile(const FString &FilePath)
{
    ClearMesh();
...
bool AObjMeshActor::ParseFaceVertex(const FString &Token, FObjIndex &OutIndex, int32 VertexCount, int32 UVCount, int32 NormalCount) const
{
...
int32 AObjMeshActor::ResolveIndex(int32 Index, int32 Count)
{
    if (Index > 0)
    {
        return Index - 1;
...
bool AObjMeshActor::BuildMesh(const TArray<FVector> &Positions, const TArray<FVector2D> &UVs, const TArray<FVector> &Normals, const TArray<TArray<FObjIndex>> &Faces)
{
